...
- **Windows support.** Post-implementation review traced `launchNotifyScript`'s spawn path and found it verified NON-functional on Windows, not merely unverified: it execs the shim directly with no shell fallback on win32 (the `/bin/sh` fallback every other bridge in this codebase relies on for exactly this class of problem is POSIX-only), and a `.cjs` file with a `#!/usr/bin/env node` shebang is not executable via `CreateProcess`. A DeepCode agent on Windows will silently get zero live status and no Stop→inbox drain — not a crash, just a permanently idle-looking card. Not fixed in this pass: the user's own environment is macOS, and building a proper cross-platform launcher (a `.cmd` wrapper invoking the bundled node, mirroring how `nodeCommand()`/`hive-node.cmd` already solve this for every other bridge) is real additional scope, tracked here as future work rather than folded into this feature.
